...
template <>
struct RpcNameTraits<&WrappedP2PMasterService::GetReadRoute> {
...
tl::expected<ReadRouteResponse, ErrorCode> P2PMasterClient::GetReadRoute(
    const ReadRouteRequest& req) {
    ScopedVLogTimer timer(1, "P2PMasterClient::GetReadRoute");
    timer.LogRequest("key=", req.key);

    auto result = invoke_rpc<&WrappedP2PMasterService::GetReadRoute,
                             ReadRouteResponse>(req);
...
tl::expected<WriteRouteResponse, ErrorCode> P2PMasterClient::GetWriteRoute(
    ...
    auto result = invoke_rpc<&WrappedP2PMasterService::GetWriteRoute,
                             WriteRouteResponse>(req);
    timer.LogResponseExpected(result);
    return result;
}
...
tl::expected<void, ErrorCode> P2PMasterClient::RemoveReplica(
    ...
    auto result = invoke_rpc<&WrappedP2PMasterService::RemoveReplica, void>(req);
    timer.LogResponseExpected(result);
    return result;
}
