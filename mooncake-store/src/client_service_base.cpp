...
void ClientServiceBase::initTeEndpoint() {
...
void ClientServiceBase::StopHeartbeat() {
    MutexLocker lk(&registration_mutex_);
    InnerStopHeartbeat();
}

void ClientServiceBase::InnerStopHeartbeat() {
...
void ClientServiceBase::Destroy() {}

bool ClientServiceBase::IsHAMode(const std::string& master_server_entry) const {
    return StartsWith(master_server_entry, kEtcdPrefix) ||
           StartsWith(master_server_entry, kRedisPrefix);
}

void ClientServiceBase::SetMasterDiscoveryConfig(
...
ErrorCode ClientServiceBase::ResolveMasterAddress(
...
void ClientServiceBase::StartHeartbeat(const std::string& master_server_entry) {
    if (heartbeat_running_) {
...
ClientServiceBase::RegisterClient() {
    MutexLocker lk(&registration_mutex_);
...
ErrorCode ClientServiceBase::ConnectToMaster(
    ...
    if (IsHAMode(master_server_entry)) {
        ...
        err = ConnectMaster(master_address);
        if (err != ErrorCode::OK) {
        ...
        auto err = ConnectMaster(master_server_entry);
        if (err != ErrorCode::OK) {
            return err;
        }
        return ErrorCode::OK;
...
ErrorCode ClientServiceBase::InitTransferEngine(
...
ErrorCode ClientServiceBase::InnerInitTransferEngine(
...
tl::expected<
    ...
    ErrorCode>
ClientServiceBase::BatchQueryIp(const std::vector<UUID>& client_ids) {
...
tl::expected<std::unordered_map<std::string, std::vector<Replica::Descriptor>>,
             ErrorCode>
ClientServiceBase::QueryByRegex(const std::string& str) {
    auto guard = AcquireInflightGuard();
    if (!guard.is_valid()) {
        LOG(ERROR) << "client is shutting down";
        return tl::unexpected(ErrorCode::SHUTTING_DOWN);
...
tl::expected<void, ErrorCode> ClientServiceBase::RegisterLocalMemory(
...
tl::expected<void, ErrorCode> ClientServiceBase::unregisterLocalMemory(
...
void ClientServiceBase::HeartbeatThreadMain(
...
void ClientServiceBase::WaitForNextHeartbeat(int interval_ms) {
...
void ClientServiceBase::HeartbeatTryRegister() NO_THREAD_SAFETY_ANALYSIS {
...
void ClientServiceBase::HandleHeartbeatResponse(
...
void ClientServiceBase::HandleHeartbeatTaskResult(
...
bool ClientServiceBase::ReconnectToMaster(bool is_ha_mode,
                                      std::string& current_master_address) {
    if (is_ha_mode) {
        ...
        auto err = ConnectMaster(current_master_address);
        if (err != ErrorCode::OK) {
...
void ClientServiceBase::RegisterHttpMethods() {
...
void ClientServiceBase::RegisterRuntimeConfigHttpMethods() {
    if (!http_server_) return;
...
void ClientServiceBase::StartHttpServer() {
    if (!http_server_) return;
    try {
        RegisterHttpMethods();
...
void ClientServiceBase::StopHttpServer() {
    if (http_server_) {
...
void ClientServiceBase::InitLocalBufferAllocator(size_t pool_size,
                                             const std::string& protocol,
                                             bool use_hugepage) {
    if (pool_size == 0) {
...
tl::expected<UUID, ErrorCode> ClientServiceBase::CreateCopyTask(
    const std::string& key, const std::vector<std::string>& targets) {
    return tl::make_unexpected(ErrorCode::NOT_IMPLEMENTED);
}

tl::expected<UUID, ErrorCode> ClientServiceBase::CreateMoveTask(
    const std::string& key, const std::string& source,
    const std::string& target) {
    return tl::make_unexpected(ErrorCode::NOT_IMPLEMENTED);
}

tl::expected<QueryTaskResponse, ErrorCode> ClientServiceBase::QueryTask(
    const UUID& task_id) {
    return tl::make_unexpected(ErrorCode::NOT_IMPLEMENTED);
}

tl::expected<std::vector<TaskAssignment>, ErrorCode> ClientServiceBase::FetchTasks(
    size_t batch_size) {
    return tl::make_unexpected(ErrorCode::NOT_IMPLEMENTED);
}

tl::expected<void, ErrorCode> ClientServiceBase::MarkTaskToComplete(
