...
void RegisterP2PRpcService(
    ...
    server.register_handler<&mooncake::WrappedP2PMasterService::GetReadRoute>(
...
WrappedP2PMasterService::GetReadRoute(const ReadRouteRequest& req) {
    return execute_rpc(
        "GetReadRoute", [&] { return master_service_.GetReadRoute(req); },
        [&](auto& timer) { timer.LogRequest("key=", req.key); },
        [] { MasterMetricManager::instance().inc_get_read_route_requests(); },
        [] { MasterMetricManager::instance().inc_get_read_route_failures(); });
}
