...
tl::expected<std::vector<Segment>, ErrorCode> P2PClientMeta::GetSegments() {
    SharedMutexLocker lock(&client_mutex_, shared_lock);
    auto check_ret = InnerStatusCheck();
    if (!check_ret.has_value()) {
        LOG(ERROR) << "fail to inner check client status"
...
tl::expected<std::shared_ptr<Segment>, ErrorCode> P2PClientMeta::QuerySegment(
    const UUID& segment_id) {
    SharedMutexLocker lock(&client_mutex_, shared_lock);
    auto check_ret = InnerStatusCheck();
    if (!check_ret.has_value()) {
...
ClientHealthState P2PClientMeta::get_health_state() const {
    SharedMutexLocker lock(&client_mutex_, shared_lock);
...
std::pair<ClientStatus, ClientStatus> P2PClientMeta::InnerUpdateHealthStatus() {
...
void P2PClientMeta::OnDisconnected() {
    ...
    LOG(INFO) << "the client is disconnected"
              << ", client_id=" << client_id_;
    // P2P has no architecture-specific disconnect action (the former
    // DoOnDisconnected override was a no-op).
...
void P2PClientMeta::OnRecovered() {
    ...
    LOG(INFO) << "the client is recovered"
              << ", client_id=" << client_id_;
    // P2P has no architecture-specific recover action (the former
    // DoOnRecovered override was a no-op).
