...
#include "p2p/client/tiered_cache/tiers/dram_tier.h"
#ifdef USE_ASCEND_CACHE_TIER
#include "p2p/client/tiered_cache/tiers/ascend_tier.h"
#endif
#include "p2p/client/tiered_cache/tiers/storage_tier.h"
#include "p2p/client/tiered_cache/scheduler/client_scheduler.h"
#include "p2p/client/tiered_cache/scheduler/scheduler_factory.h"
#include "utils.h"
...
tl::expected<void, ErrorCode> TieredBackend::Transfer(std::string_view key,
    ...
    if (dest_tier_it != tiers_.end()) {
        ...
        if (dest_available < required_size) {
            ...
            LOG(ERROR) << "Insufficient space in destination tier "
                       << dest_tier_id << " for key " << key
