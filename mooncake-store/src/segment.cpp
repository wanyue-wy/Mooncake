...
ErrorCode ScopedSegmentAccess::MountSegment(const Segment& segment,
    ...
    if (!segment.IsCentralizedSegment()) {
        LOG(ERROR) << "segment_id=" << segment.id
                   << ", error=non_centralized_segment";
        return ErrorCode::INVALID_PARAMS;
    }
    const auto& extra = segment.GetCentralizedExtra();
    const uintptr_t buffer = extra.base;
    const size_t size = segment.size;

    // Check if cxl storage is enable
    if (segment_manager_->enable_cxl_ && extra.protocol == "cxl") {
        ...
        switch (segment_manager_->memory_allocator_) {
            case BufferAllocatorType::CACHELIB:
                allocator = std::make_shared<CachelibBufferAllocator>(
                    segment.name, buffer, size, extra.te_endpoint, segment.id);
                break;
...
ErrorCode ScopedSegmentAccess::CommitUnmountSegment(
    ...
    if (segment != segment_manager_->mounted_segments_.end()) {
        ...
        is_cxl = segment->second.segment.IsCentralizedSegment() &&
                 segment->second.segment.GetCentralizedExtra().protocol ==
                     "cxl";
    }
...
void SegmentManager::initializeCxlAllocator(const std::string& cxl_path,
    ...
    cxl_global_allocator_ = std::make_shared<CachelibBufferAllocator>(
        cxl_path, DEFAULT_CXL_BASE, cxl_size, cxl_path, UUID{0, 0});
}
}  // namespace mooncake
