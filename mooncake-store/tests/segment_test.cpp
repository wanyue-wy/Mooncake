   ...
   protected:
    ...
    void ValidateMountedSegments(
        ...
        for (size_t i = 0; i < client_ids.size(); i++) {
            ...
            ASSERT_EQ(
                std::get<CentralizedSegmentExtraData>(seg->extra).base,
                std::get<CentralizedSegmentExtraData>(segments[i].extra).base);
    ...
    void ValidateMountedSegment(
        const CentralizedSegmentManager& segment_manager, const Segment segment,
...
TEST_F(SegmentTest, MountSegmentSuccess) {
    ...
    segment.extra = CentralizedSegmentExtraData{.base = 0x100000000};
...
TEST_F(SegmentTest, MountSegmentDuplicate) {
    ...
    segment2.extra = CentralizedSegmentExtraData{
        .base = std::get<CentralizedSegmentExtraData>(segment.extra).base +
                segment.size};
...
TEST_F(SegmentTest, UnmountSegmentSuccess) {
    ...
    ASSERT_EQ(segment_manager.PrepareUnmountSegment(
                  segment.id, metrics_dec_capacity, segment.name),
              ErrorCode::OK);
