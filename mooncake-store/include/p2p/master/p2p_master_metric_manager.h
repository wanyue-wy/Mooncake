...
// Standalone master-side metrics for the P2P architecture.
//
// Self-contained: the generic master metrics formerly inherited from the
// MasterMetricManager base class are now owned directly by this class
// (metric names/labels unchanged, so Prometheus scrapes and dashboards keep
// working). The centralized architecture keeps its own metric manager; a
// process runs a single architecture.
class P2PMasterMetricManager {
   public:
    static P2PMasterMetricManager& instance();

    // Resets all metrics to the initial state (for tests).
   ...
   private:
    ...
    // Marks all P2P metrics as changed once so zero values are serialized.
